...
- **DOCX file support** (`feature/docx-input`): users can now upload, preview, and parse `.docx` files in addition to `.pdf`. Before passing to Docling, the DOCX is converted to PDF via LibreOffice headless — this gives the analysis pipeline a real page model with bounding boxes (correct multi-page support, element overlays in the viewer) while keeping the DOCX file available for download. The converted PDF is cached on disk as a companion file (`<uuid>.pdf` alongside `<uuid>.docx`) so LibreOffice runs **at most once per document**: whichever happens first — analysis or first preview request — creates the companion; every subsequent preview reads it directly from disk. The companion is deleted alongside the original when the document is removed. The download dropdown adapts dynamically to the document's source format (PDF documents offer PDF download, DOCX documents offer DOCX download — never both). All upload entry points (`DocumentUpload`, `DocsNewPage`, `ReasoningDocPicker`) accept `.pdf` and `.docx`; unsupported extensions are rejected. `libreoffice-writer-nogui` added to the Docker image.
