...
- **Reasoning Trace v2 — timeline + Ask panel in the Parse view** (#303): live reasoning runs in the Parse view of an analysis (*Analyses* → open a run) instead of a standalone page. Each step carries a typed kind — the wire contract reserves `PLAN`/`RETRIEVE`/`RERANK`/`READ`/`VERIFY`/`ANSWER`/`MAP` so future agent phases render without a contract change, though docling-agent 0.6.0's chunkless loop emits only `READ` today. The run's wall-clock total is captured server-side; docling-agent 0.6.0 reports no per-step duration, so the timeline renders in its uniform "step-order" mode with a footnote rather than drawing bars. A conversational Ask panel (composer + turn cards) drives the runs. Focusing a step reveals and scrolls the linked element across the PDF, the document tree, and the Parse view.
...
- **Lean development stack**: `docker-compose.dev.yml` now gates every service behind a profile — `COMPOSE_PROFILES=default docker compose -f docker-compose.dev.yml up` starts only the frontend and the SQLite-backed parser, while `graph` adds Neo4j, `ingestion` adds OpenSearch + Dashboards + embedding, and `remote` adds Docling Serve. **A bare `docker compose -f docker-compose.dev.yml up` now starts nothing** — `COMPOSE_PROFILES` is mandatory. The production `docker-compose.yml` is unchanged.
...
- **The Stores management UI**: the stores list, detail, create, edit and query pages and their routes are removed (~1 500 lines). Store CRUD remains available over the API (`/api/stores/*`) and the ingestion pipeline is unchanged.
